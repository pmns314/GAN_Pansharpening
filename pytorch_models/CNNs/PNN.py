...
if __name__ == '__main__':
    ...
    model = PNN(train_data.channels)
    model.to(device)

    # Load Pre trained Model
    model.load_state_dict(torch.load(ROOT_DIR + "/pytorch_models/trained_models/pnn.pth", map_location=torch.device(device)))

    loss_fn = torch.nn.MSELoss(size_average=True).to(device)
